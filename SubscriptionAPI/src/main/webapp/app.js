document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (Array.isArray(subscriptions) && subscriptions.length > 0) {
                let table = '<div style="overflow-x:auto;"><table class="subs-table"><thead><tr>' +
                    '<th>First Name</th><th>Last Name</th><th>Email</th><th>Phone</th><th>Type</th><th>Channels</th>' +
                    // Voeg status header toe
                    '<th>Status</th>' +
                ...
                subscriptions.forEach(sub => {
                    ...
                    table += `<tr>` +
                        ...
                        `<td><span class="status-badge ${sub.active ? 'active' : 'inactive'}">${sub.active ? 'Active' : 'Inactive'}</span></td>` +

                        `<td style='min-width:160px; display:flex; gap:0.5em; align-items:center; flex-wrap:wrap;'>` +
                        `<button class='edit-btn' data-id='${sub.id}'>Edit</button>` +
                        `<button class='delete-btn' data-id='${sub.id}'>Delete</button>` +

                        // Voeg toggle knop toe
                        `<button class='toggle-btn' data-id='${sub.id}' data-active='${sub.active}'>${sub.active ? 'Deactivate' : 'Activate'}</button>` +
                        `</td></tr>`;
                });
                ...
                // Voeg event listener toe voor toggle knoppen
                document.querySelectorAll('.toggle-btn').forEach(btn => {
                    btn.addEventListener('click', async (e) => {
                        ...
                        if (isActive) {
                            await fetch(`${API_BASE}/${id}/deactivate`, { method: 'POST' });
                        } else {
            ...
            subscriptionsDiv.innerHTML = searchHtml + '<div style="color:red; padding:1em; background:#fff3f3; border:1px solid #f5c2c7; border-radius:0.5em;">'+
                ...
                'Please check if the backend server is running and accessible at <b>http://localhost:8080/api/subscriptions</b>.'+
    ...
    const NOTES_API_BASE = 'http://localhost:8080/api/subscription-notes';
    // Attach addNote to the window
    window.addNote = async function () {
        const subscriptionId = document.getElementById('subscriptionId').value;
        const noteText = document.getElementById('noteText').value;

        if (!subscriptionId || !noteText.trim()) {
            alert("Subscription ID and Note text are required.");
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                await window.fetchNotes(); // Refresh the displayed notes
            } else {
                const errorText = await response.text();
                alert("Error adding note: " + errorText);
            }
        } catch (error) {
            alert("Error: " + error);
        }
    ...
    // Attach fetchNotes to the window
    window.fetchNotes = async function () {
        const subscriptionId = document.getElementById('subscriptionIdId').value;
        const notesContainer = document.getElementById('notesContainer');

        if (!subscriptionId) {
            notesContainer.innerHTML = "<p>Please enter a Subscription ID.</p>";
            return;
        }

        try {
            const response = await fetch(`${NOTES_API_BASE}/${subscriptionIdId}`);
            const notes = await response.json();

            if (notes.length > 0) {
                let notesHtml = "<ul>";
                notes.forEach(note => {
                    notesHtml += `<li><strong>Note:</strong> ${note.note} (Added: ${note.createdAt || 'Unknown date'})</li>`;
                });
                notesHtml += "</ul>";
                notesContainer.innerHTML = notesHtml;
            } else {
                notesContainer.innerHTML = "<p>No notes found for this Subscription ID.</p>";
            }
        } catch (error) {
            notesContainer.innerHTML = `<p>Error fetching notes: ${error}</p>`;
        }
    };
});